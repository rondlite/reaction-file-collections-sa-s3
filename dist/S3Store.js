...
(0, _debug.default)("at the filemaker s3 store so you know");
...
class S3Store extends _fileCollectionsSaBase.default {
  ...
  {}) {
    ...
    "s3");(0, _debug.default)('making new S3');this.s3 = new _s.default({ apiVersion: "2006-03-01", endpoint: process.env.AWS_S3_ENDPOINT });this.collectionName = `${collectionPrefix}${name}`.trim();this.objectACL = objectACL;}_fileKeyMaker(fileRecord) {const info = fileRecord.infoForCopy(this.name);(0, _debug.default)("S3Store _fileKeyMaker fileRecord info:", info);(0, _debug.default)("S3Store _fileKeyMaker fileRecord size:", fileRecord.size());const result = { _id: info.key || fileRecord._id, filename: info.name || fileRecord.name() || `${fileRecord.collectionName}-${fileRecord._id}`, size: info.size || fileRecord.size() };(0, _debug.default)("S3Store _fileKeyMaker result:", result);return result;} /**
